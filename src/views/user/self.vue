<template>
	<div>
		<Card dis-hover>
			...
			<div style="max-width:520px">
				<Form ref="userForm" :model="userForm" :label-width="100" label-position="right" :rules="userRules">
					...
					<FormItem label="笔名：" prop="author_name">
						<Input v-model="userForm.author_name"></Input>
					</FormItem>
					<FormItem label="密码：">
		...
		<Modal v-model="showPasswordModal" :closable='false' :mask-closable=false :width="500">
			...
			<div slot="footer">
				<Button type="warning" :loading="loadingSavePass" @click="saveAll">提交保存</Button>
...
import { apiUserPass, apiUserEdit } from "@/api/user";
export default {
	data() {
		...
		return {
			...
			userRules: {
				...
				author_name: [{ required: true, message: "请输入笔名", trigger: "blur" }]
			},
		...
		saveUser() {
			this.$refs["userForm"].validate(valid => {
				if (valid) {
					...
					apiUserEdit(this.userForm).then(res => {
		...
		savePass() {
			this.$refs["passForm"].validate(valid => {
				if (valid) {
					...
					let passObj = {
						opass: this.passForm.oldPass,
						npass: this.passForm.newPass,
					};
					apiUserPass(passObj).then(res => {
						this.loadingSavePass = false;
						if (res.code == 200) {
							this.$Message.success({
